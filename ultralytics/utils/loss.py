...
CE = torch.nn.BCEWithLogitsLoss(reduction='sum')
...
class Mutual_info_reg(nn.Module):
    def __init__(self, input_channels, channels, latent_size):
        ...
        self.fc1_depth3 = nn.Linear(channels * 1 * 4 * 4, latent_size)
        self.fc2_depth3 = nn.Linear(channels * 1 * 4 * 4, latent_size)
    ...
    def forward(self, rgb_feat, depth_feat):
        ...
        depth_feat = depth_feat.float()
        if rgb_feat.shape[1] == 256:
            rgb_feat=self.upsample05(rgb_feat)
            depth_feat=self.upsample05(depth_feat)
        if rgb_feat.shape[1] == 128:
            rgb_feat=self.upsample025(rgb_feat)
            depth_feat=self.upsample025(depth_feat)
        ...
        if rgb_feat.shape[2] == 16:
            ...
            rgb_feat = rgb_feat.view(-1, self.channel * 1 * 4 * 4)
            depth_feat = depth_feat.view(-1, self.channel * 1 * 4 * 4)
            # print(rgb_feat.shape)
...
class v8DetectionLoss:
    ...
    def __init__(self, model):  # model must be de-paralleled
        ...
        self.mutual_info_reg1 = Mutual_info_reg(512, 4, 80).to(device)
        self.mutual_info_reg2 = Mutual_info_reg(256, 4, 80).to(device)
        self.mutual_info_reg3 = Mutual_info_reg(128, 4, 80).to(device)
    ...
    def __call__(self, preds, batch):
        ...
        #     feat = []  # 或者赋值为其他适合的默认值
        # else:
        #     feat = [f.to(self.device) for f in feat]
        # feat = [f.to(self.device) for f in feat]
        # 假设 preds 是在某个阶段被定义的
        # print("Preds content:", preds)  # 打印 preds 的内容
        # print("Preds type:", type(preds))  # 打印 preds 的类型
        # if isinstance(preds, tuple):
        #     print("Preds length:", len(preds))  # 如果是元组，打印长度
        #     for i, p in enumerate(preds):
        #         print(f"Preds[{i}] shape:", p.shape if hasattr(p, 'shape') else None)  # 打印每个元素的形状

        # 判断 preds 是否为元组，并根据长度选择 feats
        feats = preds[0] if len(preds[0]) == 3 else preds[0][1]
        # feats = preds[0][1] if isinstance(preds, tuple) and len(preds) == 3 else preds[0]
        # feats = preds[1] if isinstance(preds, tuple) else preds
        # for i, xi in enumerate(feats):
        #     print(f"feats[{i}]: shape={xi.shape}, total_elements={xi.numel()}")
        ...
        mi_loss1 = self.mutual_info_reg1(preds[1][0], preds[1][1])
        # mi_loss2 = self.mutual_info_reg2(preds[1][2], preds[1][3])
        # mi_loss3 = self.mutual_info_reg3(preds[1][4], preds[1][5])
        mi_loss1=mi_loss1
        if mi_loss1<0:
            mi_loss1=0
        # loss[3] =mi_loss1*0.005        
        loss[3] =mi_loss1*0.0001        

        # Add mutual information loss to the total loss (with a weight factor)
        total_loss = loss.sum() * batch_size 

        # return loss.sum() * batch_size, loss.detach()  # loss(box, cls, dfl)
        return total_loss, loss.detach()  # Return total loss, detection losses, and mutual info loss
