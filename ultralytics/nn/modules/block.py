...
class ChannelAttentionPerFeature(nn.Module):
    def __init__(self, channels, reduction=16):
        super(ChannelAttentionPerFeature, self).__init__()
        # 输入通道数：channels，输出权重数：4
        self.fc = nn.Linear(4, 4, bias=False)
        self.softmax = nn.Softmax(dim=1)
        self.sigmoid=nn.Sigmoid()

    def forward(self, img_feat, depth_feat, spatial_attentioned_img_feat, spatial_attentioned_depth_feat):
        # 将四个特征堆叠成一个张量，维度：[batch_size, 4, channels, height, width]
        combined_feat = torch.stack([img_feat, depth_feat, spatial_attentioned_img_feat, spatial_attentioned_depth_feat], dim=1)
        # print("combined_feat Shape:", combined_feat.shape)
        # 对每个特征进行全局平均池化，得到每个特征的表示：[batch_size, 4, 1, 1]
        gap = combined_feat.mean(dim=(2, 3,4))  # [batch_size, 4]
        # print("gap Shape:", gap.shape)
        # 计算权重：fc 的输入是 [batch_size, 4]，输出是 [batch_size, 4]
        weights = self.fc(gap)  # [batch_size, 4]
        # 归一化权重，得到每个特征的权重        
        # weights = self.sigmoid(weights)  

        weights = self.softmax(weights)  # [batch_size, 4]
        # print("weights Shape:", weights.shape)
        # 将权重调整为 [batch_size, 4, 1, 1] 以便广播
        weights = weights.view(weights.size(0), 4, 1, 1)  # [batch_size, 4, 1, 1]
        
        # 对每个特征进行加权求和
        weighted_img_feat = weights[:, 0:1] * img_feat  # [batch_size, 1, channels, height, width]
        weighted_depth_feat = weights[:, 1:2] * depth_feat  # [batch_size, 1, channels, height, width]
        weighted_spatial_attentioned_img_feat = weights[:, 2:3] * spatial_attentioned_img_feat  # [batch_size, 1, channels, height, width]
        weighted_spatial_attentioned_depth_feat = weights[:, 3:4] * spatial_attentioned_depth_feat  # [batch_size, 1, channels, height, width]

        # 合并加权特征，维度为 [batch_size, channels, height, width]
        fusion_feat = weighted_img_feat + weighted_depth_feat + weighted_spatial_attentioned_img_feat + weighted_spatial_attentioned_depth_feat
...
#         super(ChannelAttentionPerFeature, self).__init__()
#         # 注意力模块用于每个特征，但每个特征的通道权重是独立的
#         self.img_feat_att = nn.Sequential(
#             nn.AdaptiveAvgPool2d(1),
#             nn.Conv2d(channels, 1, kernel_size=1, stride=1, padding=0),  # 输出通道数为1
#             nn.Sigmoid()
#         )
        
#         self.depth_feat_att = nn.Sequential(
#             nn.AdaptiveAvgPool2d(1),
#             nn.Conv2d(channels, 1, kernel_size=1, stride=1, padding=0),
#             nn.Sigmoid()
#         )
        
#         self.spatial_img_feat_att = nn.Sequential(
#             nn.AdaptiveAvgPool2d(1),
#             nn.Conv2d(channels, 1, kernel_size=1, stride=1, padding=0),
#             nn.Sigmoid()
#         )
        
#         self.spatial_depth_feat_att = nn.Sequential(
#             nn.AdaptiveAvgPool2d(1),
#             nn.Conv2d(channels, 1, kernel_size=1, stride=1, padding=0),
#             nn.Sigmoid()
#         )
...
#     def forward(self, img_feat, depth_feat, spatial_attentioned_img_feat, spatial_attentioned_depth_feat):
#         # 为每个特征计算通道权重，输出形状为 (B, 1, 1, 1)
#         img_weight = self.img_feat_att(img_feat)     # `img_feat`的通道权重
#         depth_weight = self.depth_feat_att(depth_feat)  # `depth_feat`的通道权重
#         spatial_img_weight = self.spatial_img_feat_att(spatial_attentioned_img_feat)  # `spatial_attentioned_img_feat`的通道权重
#         spatial_depth_weight = self.spatial_depth_feat_att(spatial_attentioned_depth_feat)  # `spatial_attentioned_depth_feat`的通道权重

#         # 每个特征的通道权重应用到该特征的所有通道上
#         weighted_img_feat = img_weight * img_feat
#         weighted_depth_feat = depth_weight * depth_feat
#         weighted_spatial_img_feat = spatial_img_weight * spatial_attentioned_img_feat
#         weighted_spatial_depth_feat = spatial_depth_weight * spatial_attentioned_depth_feat
        
#         # 融合特征
#         fusion_feat = weighted_img_feat + weighted_depth_feat + weighted_spatial_img_feat + weighted_spatial_depth_feat

#         return fusion_feat


class CrossAtten128(nn.Module):  
    
    def __init__(self, arg):
        super(CrossAtten128,self).__init__()
        # 128 256 512
        self.arg = arg
        self.conv1_spatial1 = nn.Conv2d(128, 1, 3, 1, 1,bias=True)
        self.conv1_spatial2 = nn.Conv2d(128, 1, 3, 1, 1,bias=True)
        self.channel_attention = ChannelAttentionPerFeature(128)
    def forward(self, x):
        img_feat=x[0]
        depth_feat=x[1]
        img_att = torch.sigmoid(self.conv1_spatial1(img_feat))
        depth_att = torch.sigmoid(self.conv1_spatial2(depth_feat))
        spatial_attentioned_img_feat = depth_att * img_feat
        spatial_attentioned_depth_feat = img_att * depth_feat
        fusion_feat=self.channel_attention(img_feat,depth_feat,spatial_attentioned_img_feat, spatial_attentioned_depth_feat)
        return fusion_feat

class CrossAtten256(nn.Module):  
    def __init__(self, arg):
        super(CrossAtten256,self).__init__()
        # 128 256 512
        self.arg = arg
        self.conv1_spatial1 = nn.Conv2d(256, 1, 3, 1, 1,bias=True)
        self.conv1_spatial2 = nn.Conv2d(256, 1, 3, 1, 1,bias=True)
        self.channel_attention = ChannelAttentionPerFeature(256)
    def forward(self, x):
        img_feat=x[0]
        depth_feat=x[1]
        img_att = torch.sigmoid(self.conv1_spatial1(img_feat))
...
class CrossAtten512(nn.Module):  
    
    def __init__(self, arg):
        super(CrossAtten512,self).__init__()
        # 128 256 512
        self.arg = arg
        self.conv1_spatial1 = nn.Conv2d(512, 1, 3, 1, 1,bias=True)
        self.conv1_spatial2 = nn.Conv2d(512, 1, 3, 1, 1,bias=True)
        self.channel_attention = ChannelAttentionPerFeature(512)
    def forward(self, x):
        img_feat=x[0]
        depth_feat=x[1]
        img_att = torch.sigmoid(self.conv1_spatial1(img_feat))
